    ...
    <div className="flex flex-col h-full space-y-6 md:space-y-10">
        ...
        <div className="flex flex-col gap-2 md:col-span-2 lg:col-span-1">
          ...
          <div className="flex items-center gap-3 bg-slate-800/20 border border-slate-700/30 px-4 py-2 rounded-xl w-fit">
            ...
            <span className="text-xs text-slate-300 font-bold whitespace-nowrap">
              {new Date(sale.startTime).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              })}{" "}
              -{" "}
              {new Date(sale.endTime).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              })}
            </span>
      ...
      <div className="flex-grow flex flex-col justify-center space-y-4">
        ...
        <p className="text-slate-400 text-base md:text-lg max-w-md leading-relaxed">
          Premium high-performance equipment designed for the elite. Available
          only during this exclusive flash window.
        </p>
      </div>
      ...
      <div className="space-y-4 pt-6 border-t border-slate-800/50">
        <div className="flex justify-between items-end px-1">
          ...
          <p
            className={`text-xs font-black uppercase tracking-widest transition-all duration-300 transform ${stockPercentage < 20 ? "text-rose-500 scale-110" : "text-cyan-500"}`}
          >
        ...
        <div className="relative h-3 bg-slate-900/80 rounded-full overflow-hidden border border-slate-800/50">
          <div
            className={`absolute inset-0 opacity-20 blur-sm animate-pulse-glow ${
              stockPercentage > 50
                ? "bg-cyan-500"
                : stockPercentage > 20
                  ? "bg-amber-500"
          ...
          <div
            className={`h-full transition-all duration-1000 ease-out relative shadow-[0_0_15px_rgba(6,182,212,0.3)] ${
              stockPercentage > 50
